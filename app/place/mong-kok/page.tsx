...
export default function MongKokPage() {
  ...
  return <main className={`mong-kok-experience mk-${active?.toLowerCase() ?? 'idle'} mk-density-${density > .58 ? 'high' : density > .25 ? 'mid' : 'low'}`}>
    {!soundEnabled && <div className="mk-sound-gate"><p>SOUND IS PART OF THE STREET</p><button onClick={enableSound}>ENTER WITH SOUND</button><button className="mk-skip-sound" onClick={() => setSoundEnabled(true)}>CONTINUE SILENTLY</button><Link className="mk-return-home" href="/">RETURN HOME ↗</Link></div>}
