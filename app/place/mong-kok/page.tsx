...
export default function MongKokPage() {
  ...
  return <main className={`mong-kok-experience mk-${active?.toLowerCase() ?? 'idle'} mk-density-${density > .58 ? 'high' : density > .25 ? 'mid' : 'low'}`}>
    ...
    <section className="mk-overload"><p className="mk-section-label">SECTION 02 / INFORMATION OVERLOAD</p><div className="mk-grid-lines" aria-hidden="true"/><div className="mk-fragments">{fragments.map((item,i) => <span key={item} className={`mk-fragment mk-fragment-${i+1}`}>{item}</span>)}</div><p className="mk-overload-copy">The street is not asking<br/><em>for your attention.</em><br/>It already has it.</p><figure className="mk-image mk-image-a"><img src="/mong-kok-signage.png" alt="Abstract layers of Mong Kok signage"/><span>旺角 / 18:47</span></figure><figure className="mk-image mk-image-b"><img src="/mong-kok-crowd.png" alt="Abstract crowd movement in Mong Kok"/><span>KEEP MOVING</span></figure></section>
    ...
    <section className="mk-human"><p className="mk-section-label">SECTION 04 / HUMAN MOMENT</p><div className="mk-human-photo"><img src="/mong-kok-human-detail.png" alt="Abstract human detail in a Mong Kok street"/><span>ONE PERSON / MANY SIGNALS</span></div><div className="mk-question">In a city this loud,<br/><em>what do you notice first?</em></div><div className="mk-choices">{([['energy','The energy'],['people','The people'],['noise','The noise'],['details','The details']] as [Choice,string][]).map(([v,label],i) => <button key={v} className={choice===v?'is-selected':''} onClick={() => choose(v)}><small>0{i+1}</small>{label}</button>)}</div>{choice && <p className="mk-saved">YOUR HONG KONG MOOD / SAVED</p>}</section>
